...
public enum CSSqlMethod {
    ...
    SELECT_LIST_WITH_JOIN("selectListWithJoin", "查询满足条件所有对象（带表连接）", "<script>%s SELECT %s FROM %s %s %s %s\n</script>"),
    SELECT_PAGE_WITH_JOIN("selectPageWithJoin", "查询满足条件的分页数据（带表连接）", "<script>%s SELECT %s FROM %s %s %s %s\n</script>"),
    SELECT_MAPS_WITH_JOIN("selectMapsWithJoin", "查询满足条件所有数据（带表连接）", "<script>%s SELECT %s FROM %s %s %s %s\n</script>"),
