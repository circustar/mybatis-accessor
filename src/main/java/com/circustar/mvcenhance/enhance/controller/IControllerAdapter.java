...
import com.circustar.mvcenhance.common.error.*;
...
//            result.okResult(null);
            ...
            if (dto != null && !StringUtils.isEmpty(sub_entities)) {
                Map<String, EntityFilter[]> tableJoinerMap = Arrays.asList(sub_entities
                        .split(",")).stream()
                        .collect(Collectors.toMap(x -> x, y -> {
                            EntityFilter[] t = null;
                            try {
                                t = FieldUtils.getFieldAnnotationByName(dto, FieldUtils.unCap(y), EntityFilter.class);
                ...
                for (String fieldName : tableJoinerMap.keySet()) {
                    ...
                    if (entityFilters == null || entityFilters.length == 0) {
